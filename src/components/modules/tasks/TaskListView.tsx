...
function PriorityGroup({
  priority, tasks, onComplete, onUncomplete, onDelete, onEdit,
}: {
  priority: number;
  tasks: Task[];
  ...
  return (
    <div>
      ...
      >
        ...
        <span className={`font-ui text-xs font-semibold uppercase tracking-wide ${cfg.text}`}>{cfg.label}</span>
        <span className={`font-ui text-xs font-semibold px-1.5 py-0.5 rounded-full ${cfg.pill}`}>{tasks.length}</span>
...
function CompletedTodaySection({ tasks, onComplete, onUncomplete, onDelete, onEdit }: {
  tasks: Task[];
  onComplete: (id: string) => void;
  onUncomplete: (id: string) => void;
  onDelete: (id: string, snap: Task) => void;
  ...
  return (
    <div className="rounded-2xl border border-sage-200 dark:border-sage-800/40 overflow-hidden">
      <button
        onClick={() => setOpen(p => !p)}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-ui text-sm font-bold text-sage-700 dark:text-sage-300">Concluídas hoje</span>
          <span className="font-ui text-xs font-semibold px-1.5 py-0.5 rounded-full bg-sage-100 text-sage-700 dark:bg-sage-900/40 dark:text-sage-300">
            {tasks.length}
          </span>
        </div>
        {open ? <ChevronUp size={15} className="text-sage-500" /> : <ChevronDown size={15} className="text-sage-500" />}
      </button>
      <motion.div
        initial={false}
        animate={{ gridTemplateRows: open ? '1fr' : '0fr' }}
...
export function TaskListView({
  tasks, onComplete, onUncomplete, onDelete, onEdit, onInlineAdd, onUrgentClick,
}: TaskListViewProps) {
  ...
  const urgentAndOverdue = useMemo(
    () => tasks.filter(t => !t.isCompleted && (t.priority === 0 || t.isOverdue))
          ...
          {[{ value: 'all', label: 'Todas' }, ...CATEGORIES.map(c => ({ value: c.value, label: c.label }))].map(cat => {
            ...
            return (
              <button
                ...
                className={`font-ui text-xs font-semibold rounded-full px-3 py-1.5 transition-colors ${
        ...
        <div className="space-y-4">
          ...
          {totalFiltered === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="font-ui font-semibold text-base text-foreground">Tudo em dia!</p>
              <p className="font-ui text-sm mt-1">Nenhuma tarefa pendente nesta categoria.</p>
            </div>
          )}
        </div>
