...
export async function listCategories(
  ...
  const raw = await httpClient.post<unknown>(ENDPOINTS.legacyCategories.list, filter ?? {}, { nestId });
...
export async function listCategoryOptions(
  ...
  const raw = await httpClient.post<unknown>(ENDPOINTS.legacyCategories.listOptions, query ?? {}, {
    nestId,
  });
...
export async function createCategory(
  ...
  const raw = await httpClient.post<unknown>(ENDPOINTS.legacyCategories.create, payload, { nestId });
...
export async function getCategoryById(id: string, nestId?: string): Promise<CategoryResponse> {
  const raw = await httpClient.get<unknown>(`${ENDPOINTS.legacyCategories.getById}?id=${id}`, nestId);
...
export async function updateCategory(
  ...
  await httpClient.put<void>(ENDPOINTS.legacyCategories.update(id), payload, nestId);
  try {
...
export async function deleteCategory(id: string, nestId?: string): Promise<void> {
  ...
  await httpClient.del<void>(ENDPOINTS.legacyCategories.delete(id), nestId);
}
