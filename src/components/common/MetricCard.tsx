...
const MetricCard = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'duration-[length:var(--dur-base)] overflow-hidden rounded-xl border text-card-foreground shadow-sm transition-all hover:shadow-md',
    ...
    >
      ...
      {alertType && <div className={cn('h-1 w-full shrink-0', alertStripClass[alertType])} />}

      <div className="flex flex-1 flex-col p-5">
        {/* Header: Icon + Title */}
        <div className="mb-4 flex items-center space-x-3">
          <div
            className={cn('rounded-lg p-2', toneStyles?.iconBg)}
            style={!toneStyles ? { backgroundColor: `${color}20`, color } : undefined}
          >
            {toneStyles ? <Icon size={24} className={toneStyles.iconColor} /> : <Icon size={24} />}
          </div>
        ...
        <motion.div
          ...
          className={cn('mb-2 font-bold text-[var(--text-2xl)]', toneStyles?.valueColor)}
        ...
        {comparison !== undefined && (
          <div className="mb-3 flex items-center space-x-1">
            ...
            <span className="ml-1 text-xs text-muted-foreground">{comparisonLabel}</span>
          </div>
        )}
        ...
        {chartData && chartData.length > 0 && (
          <div className="mb-3">
            <MiniLineChart
              data={chartData}
              color={toneStyles?.chartHex ?? color ?? '#c1714d'}
              height={50}
            />
          </div>
        )}
