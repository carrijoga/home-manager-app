...
Components are split into four sub-directories:

| Directory | Purpose |
|-----------|---------|
| `common/` | Reusable UI primitives and shared widgets used across modules |
| `modules/` | Full feature views — one file per route (Dashboard, Tasks, Shopping, etc.) |
| `skeletons/` | Loading skeleton variants for each major view |
...
`Navigation.tsx` and `app-sidebar.tsx` live directly under `src/components/` — they are layout-level components, not feature modules.

## Naming Conventions

- `common/` — PascalCase, descriptive noun: `MetricCard`, `PostIt`, `BulletinBoard`
- `modules/` — PascalCase matching route: `Dashboard`, `Tasks`, `ShoppingList`
- `skeletons/` — `<Module>Skeleton`: `DashboardSkeleton`, `ListSkeleton`
- Modal/sheet names end with `Modal` or `Sheet`: `SettingsModal`, `NestManagerSheet`

## File Extension Rules

- New components: always `.tsx`
- Existing `.jsx` module files: migrate to `.tsx` gradually — do not rewrite wholesale, only when editing
- `ui/` components: always `.tsx` (they already are)
...
**Never use raw native `<select>`.** It ignores the app's theming and has no shared styling hook. Two legacy usages remain (`modals/BankAccountSheet.tsx`, `modules/FutureItems.jsx`) — migrate them when you next touch those files. Use the Radix `Select` from `@/components/ui`:

```tsx
<Select value={value} onValueChange={onChange}>
  <SelectTrigger id="field-id" className="bg-muted/30 border-border/40">
...
- Load the data the field needs (e.g. `PaymentCard` loads bank accounts via `bankAccountService.listBankAccounts` before the sheet can require one)
...
- Keep layout logic in the module file; extract sub-components to `common/` only when reused elsewhere

Currently in `.jsx` (migration candidates): `Dashboard`, `Tasks`, `Calendar`, `Financial`, `FutureItems`, `DashboardTasksSection`.
