...
const FutureItems = memo(() => {
  ...
  useEffect(() => {
    ...
    futureItemsService
      .getAllFutureItems()
      .then((data) => {
        if (isMounted) setFutureItems(data);
      })
      .catch(() => {});
    return () => {
      isMounted = false;
      ...
      <Card title="Itens para Comprar no Futuro">
        ...
        <div className="mb-6 space-y-3 rounded-lg border border-border bg-muted/50 p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <SelectTrigger id="future-priority" className="border-border/40 bg-muted/30">
            ...
            return (
              <div key={priority}>
                ...
                {itemsInPriority.map((item) => (
                  <div
                    key={item.id}
                    className={`mb-2 flex items-center justify-between rounded-lg border-l-4 p-4 transition-all duration-200 hover:opacity-90 ${priorityColors[priority]}`}
                  >
                    ...
                    <button
                      ...
                      className="text-rose-500 transition-colors hover:text-rose-700 dark:text-rose-400 dark:hover:text-rose-300"
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
