...
function dedupeCategories(cats: AppShoppingCategory[]): AppShoppingCategory[] {
  ...
  cats.forEach((c) => { if (!map.has(c.shoppingCategoryId)) map.set(c.shoppingCategoryId, c); });
...
export function useShoppingData() {
  ...
  const createShoppingList = useCallback(async (name: string, monthYear: string, notes?: string) => {
  ...
  const updateShoppingList = useCallback(async (id: string, name: string, monthYear: string, notes?: string) => {
  ...
  const finishShoppingList = useCallback(async (id: string) => {
  ...
  const unfinishShoppingList = useCallback(async (id: string) => {
  ...
  const deleteShoppingItem = useCallback(async (
    id: string, listId: string, quantity: number, unitType: number,
    estimatedPrice?: number | null, isPurchased?: boolean, price?: number | null,
  ) => {
      ...
      prev.map((l) =>
        l.shoppingListId === listId
          ? {
              ...
              totalEstimated: Math.max(0, (l.totalEstimated ?? 0) - (estimatedPrice ?? 0) * mult),
              totalSpent: isPurchased ? Math.max(0, (l.totalSpent ?? 0) - (price ?? 0) * mult) : l.totalSpent,
            }
          : l
      )
    );
  ...
  const markItemAsPurchased = useCallback(async (
    id: string, listId: string, quantity: number, unitType: number, price: number, purchasedAt: string,
  ) => {
    await shoppingService.markItemAsPurchased(id, { price, purchasedAt }, nestId);
    const spentContrib = price * (isPricePerUnit(unitType) ? quantity : 1);
    setShoppingLists((prev) =>
      prev.map((l) =>
        l.shoppingListId === listId
          ? { ...l, purchasedItems: l.purchasedItems + 1, totalSpent: (l.totalSpent ?? 0) + spentContrib }
          : l
      )
    );
  }, [nestId]);

  const unmarkItemAsPurchased = useCallback(async (
    id: string, listId: string, quantity: number, unitType: number, price: number,
  ) => {
  ...
  const uploadShoppingItems = useCallback(async (listId: string, file: File): Promise<AppShoppingList> => {
  ...
  const createShoppingCategory = useCallback(async (name: string, description?: string): Promise<AppShoppingCategory> => {
